    ...
    try {
        ...
        if (req_body.original_pair == req_body.order_pair) {
            ...
            if (flag == 1) {
                ...
                // console.log("faldax_fee", req_body)
                ...
                if (get_jst_price[0].coin == null) {
                    ...
                    // if (!usd_value) {
                    //     priceValue = (1 / cryptoUSD[0].ask_price);
                    // } else {
                    priceValue = (1 / cryptoUSD[0].ask_price);
                    // }
                } else {
                ...
                // }
                req_body.OrderQty = usd_price * priceValue;

                totalValue = req_body.OrderQty;
                ...
                if (get_jst_price[0].coin == null) {
                    var cryptoUSD = await getLatestPrice.latestPrice(crypto + 'USD', (req_body.Side == 1 ? "Buy" : "Sell"));
                    var currencyUSD = await getLatestPrice.latestPrice(currency + 'USD', (req_body.Side == 1 ? "Buy" : "Sell"));
                    priceValue = (1 / currencyUSD[0].ask_price);
                } else {
                    if (req_body.Side == 1) {
                        priceValue = (get_jst_price[0].ask_price);
                    }
        ...
        returnData.limit_price = parseFloat(returnData.limit_price).toFixed(8)
